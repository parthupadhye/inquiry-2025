/**
 * ClaimVerifyWorkflow Executor
 * Executes the claim-verify workflow.
 *
...
/**
 * Executor for ClaimVerifyWorkflow.
 */
export class ClaimVerifyWorkflowExecutor {
  ...
  async start(
    ...
    this.context = {
      ...
      system: {
        executionId,
        workflowId: claimVerifyWorkflowDefinition.id,
        workflowVersion: claimVerifyWorkflowDefinition.version,
        startedAt: now,
        currentStepId: null,
      },
      stepResults: {},
  ...
  async execute(): Promise<WorkflowResult> {
    ...
    try {
      // Execute steps
      for (const step of claimVerifyWorkflowSteps) {
  ...
  private async executeStep(stepId: string): Promise<StepResult> {
    ...
    const step = claimVerifyWorkflowSteps.find((s) => s.id === stepId);
    if (!step) {
   ...
   */
  private async executeStepByType(step: (typeof claimVerifyWorkflowSteps)[number]): Promise<Record<string, unknown>> {
    switch (step.type) {
      case 'agent':
...
/**
 * Executes the ClaimVerifyWorkflow workflow.
 */
export async function executeClaimVerify(
  input: ClaimVerifyInput,
  options: StartWorkflowOptions = {}
): Promise<WorkflowResult> {
  const executor = new ClaimVerifyWorkflowExecutor();
